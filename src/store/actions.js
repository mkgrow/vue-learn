...
export const getSales = ({ commit }, { page }) => {
  reqwest({
    url: `${process.env.API_ROOT}sales?format=json&page=${page}`,
    method: 'get',
    contentType: 'application/json',
    crossOrigin: true,
    data: '参数',
    ...
    commit({
      type: 'getSales', // 这个type很重要，vue会自动去找mutations.js中名为getMsg的方法
      sales // 成功后把得到的数据通过commit传入mutations
    })
  })
}

export const getSale = ({ commit }, {id}) => {
  reqwest({
    url: `${process.env.API_ROOT}sales/${id}.json`,
    method: 'get',
    contentType: 'application/json',
    crossOrigin: true,
    data: '参数',
    ...
    commit({
      type: 'getSale', // 这个type很重要，vue会自动去找mutations.js中名为getMsg的方法
      sale // 成功后把得到的数据通过commit传入mutations
    })
  })
}
